...
* Agent.ckpt: final trained Agent model
* sampled: SMILES generated by final Agent model, along with their and Prior likelihood
* training_log_novel.npy: for the SMILES dataset generated during each training step, this is the percentage of SMILES that are unique from original QM9 dataset 
* taining_log_sa.npy: for the SMILES dataset generated during each training step, these are the synthetic accessibility scores
* training_log_scores.npy: Training score during each training step
* training_log_sigmas.npy: Sigma value used in each training step; if sigma_mode is not static, this will change over the course of Agent training
* training_log_valid.npy: for the SMILES dataset generated during each training step, this is the percentage of SMILES that are syntactically valid
