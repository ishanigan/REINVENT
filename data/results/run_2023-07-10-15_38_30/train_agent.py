...
def train_agent(restore_prior_from='data/Prior.ckpt',
    ...
    prior_likelihood = prior_likelihood.data.cpu().numpy()
    smiles = seq_to_smiles(seqs, voc)
    score = scoring_function(np.mean(score))

    np.save('training_log_sa.npy', np.array(sa))
    np.save('training_log_novel.npy', np.array(novel))
    np.save('training_log_valid.npy', np.array(valid))
    np.save('training_log_scores.npy', np.array(scores))
